...
func DeployCompressFiles(db *gorm.DB, nodeID string, environmentID string, version string) ([]byte, int, error) {
	...
	err := db.Raw(query, nodeID, environmentID, version).Scan(&FilesOutput).Error
	if err != nil {
